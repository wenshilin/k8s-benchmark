...
def draw_one_dir():
    # 数据的根目录
    base_dir = '/Volumes/Data/实验数据/final/ce-624/lrp/+25-dt/'
    root_dir = base_dir + 'jobs'
    save_dir = base_dir
...
def main():
    ...
    if len(argv) == 1:
        ...
        job_dir = argv[1]
        save_dir = argv[2]
        treat_model_as_drl = bool(int(argv[3]))
        draw_job_figures(job_dir, save_dir, treat_model_as_drl=treat_model_as_drl, show_figure=False)
